...
<div id="Title_bar">
    <div id="Title_bar_Head">
        ...
        <div id="Title"><!--页面标题-->
            <img border="0" width="13" height="13" src="<%= basePath %>images/common/title_arrow.gif"/> 修改部门信息
        </div>
...
<!--显示表单内容-->
<div id=MainArea>
    <form action="<%= basePath %>manage/editDepartment" method="post">
    <input type="hidden" id="id" name="id" value="${nowDepartment.id}"/>
        ...
        <div class="ItemBlockBorder">
            <div class="ItemBlock">
                <table cellpadding="0" cellspacing="0" class="mainForm">
                        ...
                        <option value="0" selected="selected">请选择部门</option>
                        		<c:forEach items="${departmentList}" var="parentDepartment">
                        				...
                        				<option value="${parentDepartment.id}" 
                 						<c:if test="${nowDepartment.parent.id == parentDepartment.id}">
       										selected="selected"
                        				...
                        				<c:forEach items="${departmentList}" var="childDepartment">
                        					<c:if test="${childDepartment.parent.id == parentDepartment.id}">
                        						...
                        						<c:if test="${nowDepartment.parent.id == childDepartment.id}">
                       								selected="selected"
                        						...
                        						<c:forEach items="${departmentList}" var="grandchildDepartment">
		                        					<c:if test="${grandchildDepartment.parent.id == childDepartment.id}">
		                        						...
		                        						<c:if test="${nowDepartment.parent.id == grandchildDepartment.id}">
		                       								selected="selected"
                    ...
                    <tr><td>部门名称</td>
                        <td><input type="text" name="name" id="name" class="InputStyle" value="${nowDepartment.name}"/> *</td>
                    </tr>
                    <tr><td>职能说明</td>
                        <td><textarea name="description" id="description" class="TextareaStyle">${nowDepartment.description}</textarea></td>
                    </tr>
                </table>
            </div>
        </div>
