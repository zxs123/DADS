...
<script language="javascript" src="<%=basePath%>js/common/pageCommon.js"
	charset="utf-8"></script>
<script language="javascript" src="<%=basePath%>js/common/PageUtils.js"
	charset="utf-8"></script>
...
$(document).ready(function(){
		...
		if("success" == result) {
			alert("数据提交成功  ！");	
			location.href = "<%=basePath %>report/initEditDsfpb";
...
function addData(){	
	...
	+"<td>认领员工号</td><td><input type='text' size='7'  id='loginName"+rowCount+"' name='' value='' onblur='getygname("+rowCount+")'/></td>"
	+"<td>认领员工姓名</td><td><input type='text' size='7' id='name"+rowCount+"' name='name' value=''/></td>"
	+"<td>领出员工号</td><td><input type='text' size='7'  id='lcLoginName"+rowCount+"' name='lcLoginName' value='' onblur='getlcname("+rowCount+")'/></td>"
	+"<td>领出员工姓名</td><td><input type='text' size='7' id='lcName"+rowCount+"' name='lcName' value=''/></td>"
	+"<td>占比</td><td><input type='text' size='7' id='QUANZH"+rowCount+"' name='QUANZH' value=''/></td>"
...
function getlcname(id){
	alert
	var type=4
	var lcLoginName=$.trim($("#lcLoginName"+id).val())
	if(null == lcLoginName || "" == lcLoginName) {
		return false;
	}
	var Flag = false;
	$.ajax({
...
function checkData(){
	...
	if(flag==1){
		alert("领入员工姓名获取失败，请检查员工号是否正确 !");
		return false;
	}
	
	var lcLoginName=document.getElementsByName("lcLoginName");
	for(var i=0;i<lcLoginName.length;i++){	
		if(null==$.trim(lcLoginName[i].value) || ""==$.trim($.trim(lcLoginName[i].value))){
				flag=1;
			}
	}  
	if(flag==1){
	...
	var lcName=document.getElementsByName("lcName");
	for(var i=0;i<lcName.length;i++){	
		if(null==$.trim(lcName[i].value) || ""==$.trim($.trim(lcName[i].value))){
				flag=1;
			}
	}  
	if(flag==1){
		alert("领出员工姓名获取失败，请检查员工号是否正确 !");
		return false;
	}
		...
		<div id="Title_bar_Head">
			...
			<div id="Title">
				...
				<img border="0" width="13" height="13"
					src="<%=basePath%>images/common/title_arrow.gif" /> 修改对私分配
			</div>
			<div id="Title_End"></div>
		</div>
	</div>

	<form id="searchForm" method="post" action="<%=basePath%>report/editDsfpb" >
            ...
            <div class="ItemBlock">
                <table cellpadding="0" cellspacing="0" class="mainForm" id="fpbdata">
                     ...
                     <c:if test="${ flag == 0 }">         
	         			...
	         			<td>认领员工号</td><td><input type="text" size="7"    id="loginName0" name="loginName" value="" onblur="getygname(0)"/></td>
	           			<td>认领员工姓名</td><td><input type="text" size="7"  id="name0" name="name" value=""/></td>
	           			<td>领出员工号</td><td><input type="text" size="7" id="lcLoginName0" name="lcLoginName" value="" onblur="getlcname(0)"/></td>
	           			<td>领出员工姓名</td><td><input type="text" size="7" id="lcName0" name="lcName" value=""/></td>
	           			<td>占比</td><td><input type="text" size="7"   id="QUANZH0" name="QUANZH" value=""/></td>
	           			<td>
	           		...
	           		<c:if test="${flag==1}">
	           			<c:forEach items="${resultList}" var="resultlist" varStatus="vs">
	            			...
	            			<td>认领员工号</td><td><input type="text" size="7"  id="loginName${vs.index}" name="loginName" value="${resultlist.loginName}" onblur="getygname(${vs.index})"/></td>
	           				<td>认领员工姓名</td><td><input type="text" size="7"  id="name${vs.index}" name="name" value="${resultlist.name}"/></td>
	           				<td>领出员工号</td><td><input type="text" size="7" id="lcLoginName${vs.index}" name="lcLoginName" value="${resultlist.lcLoginName}" onblur="getlcname(${vs.index})"/></td>
	           				<td>领出员工姓名</td><td><input type="text" size="7" id="lcName${vs.index}" name="lcName" value="${resultlist.lcName}"/></td>
	           				<td>占比</td><td><input type="text" size="7"  id="QUANZH${vs.index}" name="QUANZH" value="${resultlist.QUANZH}"/></td>	         	
	            	  		<td>
